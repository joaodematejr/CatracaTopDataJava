...
public class EasyInner {
	 ...
	 * dados. O resultado do cadastro deve ser verificado no retorno da função
	 * UsuarioFoiEnviado. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para
	 * comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	 * porta fixa. Template: O cadastro do usuário já contendo as duas digitais e o
	 * número do usuário. É um array de bytes com o tamanho de 844 bytes.
	 *
	 * @param Inner
	 * @param usuario
	 * @param Template1
	...
	/**
	 * Configura qual será o número do cartão master que o Inner irá aceitar. Válido
	 * somente para o padrão livre de cartão. Para o padrão Topdata o número do
	 * master sempre é 0.
	 *
	...
	/**
	 * Recebe o status atual dos sensores do Inner. Essa função deverá ser utilizada
	 * somente em casos muito específicos, por exemplo, quando você possui um Inner
	 * Plus/NET conectado a um sensor de presença e deseja saber se existe alguma
	 * pessoa naquele local Retorna o status do respectivo sensor: 0 – em nível
	 * baixo (fechado) 1 – em nível alto (aberto)
	 *
	 * @param Inner
	 * @param Sensores
	 * @return
	 ...
	 * 0 - Comunicação serial, RS-232/485. 1 - Comunicação TCP/IP com porta
	 * variável. 2 - Comunicação TCP/IP com porta fixa (Default). 3 - Comunicação
	 * via modem. 4 – Comunicação via TopPendrive.
	 *
	 * @param Tipo
	 * @return
	 */
	...
	/**
	 * Abre a porta de comunicação. essa função deverá ser chamada antes de iniciar
	 * qualquer processo de transmissão ou recepção de dados com o Inner. Esta
	 * função deve ser chamada apenas uma vez e no início da comunicação, e não deve
	 * ser chamada para cada Inner. Número da porta serial ou TCP/IP: - Para
	 * comunicação TCP/IP o valor padrão da porta é 3570 (Default). - Para
	 * comunicação Serial/Modem o valor padrão da porta é 1, COM 1(Default). - Para
	 * a comunicação TopPendrive o valor é 3(Default).
	 *
	 * @param Porta
	 * @return int
	 */
	 ...
	 * TCP/IP. Em modo Off-Line normalmente é chamada após enviar/receber todos os
	 * dados do Inner. Em modo On-Line é chamada somente no ecerramento do software
	 * do software.
	 */
	public static native void FecharPortaComunicacao();

	/**
	 * Define qual padrão de cartão será utilizado pelos Inners, padrão Topdata ou
	 * padrão livre. O padrão Topdata de cartão está descrito no manual dos
	 * equipamentos e é utilizado somente com os Inners em modo Off-Line. No padrão
	 * livre todos os dígitos do cartão são considerados como matrícula, ele pode
	 * ser utilizado no modo On Line ou no modo Off Line. Ao chamar essa função, a
	 * quantidade de dígitos é setada para 14. 0 - Padrão Topdata. 1 - Padrão livre
	 * (Default).
	 *
	 * @param Padrao
	 * @return int
	...
	/**
	 * Testa a comunicação com o Inner, também utilizado para efetuar a conexão com
	 * o Inner. Para efetuar a conexão com o Inner, essa função deve ser executada
	 * em um loop até retornar 0(zero), executado com sucesso.
	 *
	 * @param Inner
	 * @return
	 */
	...
	/**
	 * Liga a luz emitida pelo display do Inner. Essa função não deve ser utilizada
	 * nas catracas.
	 *
	 * @param Inner
	 * @return
	 ...
	 * equipamento. Habilitando o parâmetro ecoar, o teclado irá ecoar asteriscos no
	 * display do Inner. Habilita: 0 - Desabilita o teclado (Default). 1 - Habilita
	 * o teclado. Ecoar: 0 – Ecoa o que é digitado no display do Inner (Default). 1
	 ...
	 * tempo ele será acionado. Função: 0 – Não utilizado(Default). 1 – Aciona ao
	 * registrar uma entrada ou saída. 2 – Aciona ao registrar uma entrada. 3 –
	 * Aciona ao registrar uma saída. 4 – Está conectado a uma sirene(Ver as funções
	 * de sirene). 5 – Utilizado para a revista de usuários(Ver a função
	 * DefinirPorcentagemRevista). 6 – Catraca com a saída liberada. 7 – Catraca com
	 * a entrada liberada 8 – Catraca liberada nos dois sentidos. 9 – Catraca
	 * liberada nos dois sentidos e a marcação(registro) é gerada de acordo com o
	 ...
	 * barras(Default). 1 – Leitor Magnético. 2 – Leitor proximidade AbaTrack2. 3 –
	...
	/**
	 * Configura as operações que o leitor irá executar. Se irá registrar os dados
	 * somente como entrada independente do sentido em que o cartão for passado,
	 * somente como saída ou como entrada e saída. Operação: 0 – Leitor
	...
	/**
	 * Configura o Inner para registrar as tentativa de acesso negado. O Inner irá
	 * rgistrar apenas os acessos negados em relação a lista de acesso configurada
	 * para o modo Off-Line, ver as funções DefinirTipoListaAcesso e ColetarBilhete.
	 * TipoRegistro: 0 – Não registrar o acesso negado. 1 – Apenas o acesso negado.
	 * 2 – Falha na validação da digital. 3 – Acesso negado e falha na validação da
	 * digital.
	 *
	 * @param TipoRegistro
	 * @return
	 ...
	 * Desablitado(Default). 1 a 9 – Registrar como uma função do teclado do Inner.
	 * 10 – Registrar sempre como entrada. 11 – Registrar sempre como saída. 12 –
	 ...
	 * Inner, e configura se o segundo leitor irá exibir as mensagens configuradas.
	 * Habilita: 0 – Não habilita o segundo leitor como wiegand(Default). 1 –
	 * Habilita o segundo leitor como wiegand. ExibirMensagem: 0 – Não exibe
	 ...
	 * inserido o dedo no Inner bio sem que o usuário tenha definido se é um
	 * entrada, saída, função, etc. Funcao: 0 – desabilitada(Default). 1 a 9 –
	 * funções de 1 a 9. 10 – entrada. 11 – saída. 12 – libera catraca para os dois
	...
	/**
	 * Envia o buffer interno da dll que contém todas as configurações das funções
	 * anteriores para o Inner, após o envio esse buffer é limpo sendo necessário
	 * chamar novamentes as funções acima para reconfigurá-lo. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 * @return
	 */
	 ...
	 * serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a ... – Para
	 * comunicação TCP/IP porta fixa. Dia - 1 a 31 Mes - 1 a 12. Ano - 0 a 99 Hora -
	...
	/**
	 * Solicita a versão do firmware do Inner e dados como o Idioma, se é uma versão
	 * especial. Inner: Número do Inner desejado. Linha: 01 – Inner Plus. 02 – Inner
	 * Disk. 03 – Inner Verid. 06 – Inner Bio. 07 – Inner NET. Variacao - Depende da
	 * versão, existe somente em versões customizadas. VersaoAlta - 00 a 99
	 * VersaoBaixa - 00 a 99 VersaoSufixo - Indica o idioma do firmware: 01 –
	 * Português. 02 – Espanhol. 03 – Inglês. 04 – Francês.
	 *
	 * @param Inner
	 * @param Versao
	 * @return
	...
	/**
	 * Apaga o buffer com a lista de horários de acesso e envia automaticamente para
	 * o Inner. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação
	 * TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 * @return
	 */
	 ...
	 * 100 horários de acesso, para cada horário é possível definir 4 faixas de
	 * acesso para cada dia da semana. Horario - 1 a 100 – Número da tabela de
	 * horários. DiaSemana - 1 a 7 – Dia da semana a qual pertence a faixa de
	 ...
	 * o comando o buffer é limpo pela dll automaticamente. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	...
	/**
	 * Limpar o buffer com a lista de usuários cadastrados e envia automaticamente
	...
	/**
	 * Insere no buffer da dll um usuário da lista e a qual horário de acesso ele
	 * está associado. Os horários já deverão ter sido cadastrados através das
	 * funções InserirHorarioAcesso e enviados através da função
	 * EnviarHorariosAcesso para a lista ter o efeito correto. Cartao - 1 a ... –
	 * Dependo do padrão de cartão definido e da quantidade de dígitos definida.
	 * Horario - 1 a 100 – Número do horário já cadastrado no Inner. 101 – Acesso
	 * sempre liberado para o usuário. 102 – Acesso sempre negado para o usuário.
	 *
	 * @param Cartao
	 * @param Horario
	 * @return
	...
	/**
	 * Envia o buffer com os usuários da lista de acesso para o Inner, após executar
	 * o comando o buffer é limpo pela dll automaticamente. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa.
	 *
	 ...
	 * habilitar a lista de acesso é necessário preencher a lista e os horários de
	 * acesso, verificar os as funções de “Horários de Acesso” e as funções da
	 * “Lista de Acesso”. Tipo: 0 – Não utilizar a lista de acesso. 1 – Utilizar
	 * lista branca(cartões fora da lista tem o acesso negado). 2 – Utilizar lista
	 * negra(bloqueia apenas os cartões da lista).
	 *
	 * @param Tipo
	 * @return
	 */
	 ...
	 * esses horário irão tocar. É possível inserir no máximo 100 horários para a
	 ...
	 * executar a função o buffer é limpo automaticamente pela dll. Inner: 1 a 32 –
	 * Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável.
	...
	/**
	 * Libera a catraca para o usuário pode efetuar o giro na catraca em ambos os
	 * sentidos. Em modo On-Line, na função ReceberDadosOnLine o valor retornado no
	 * parâmetro “Complemento” será do tipo entrada ou saída, dependendo do sentido
	 * em que o usuário passar pela catraca. Essa função deve ser utilizada somente
	 * com Inners Catraca.
	 *
	 * @param Inner
	 * @return
	...
	/**
	 * Libera a catraca no sentido de entrada padrão do Inner, para o usuário poder
	 * efetuar o giro na catraca. Em modo On-Line, na função ReceberDadosOnLine o
	 * valor retornado no parâmetro “Complemento” será do tipo entrada.
	 *
	 * @param Inner
	 * @return
	 */
	...
	/**
	 * Libera a catraca no sentido de saída padrão do Inner, para o usuário poder
	 * efetuar o giro na catraca. Em modo On-Line, na função ReceberDadosOnLine o
	 * valor retornado no parâmetro “Complemento” será do tipo saída. Essa função
	 ...
	 * ReceberDadosOnLine o valor retornado no parâmetro “Complemento” será do tipo
	 * saída. Essa função deve ser utilizada somente com Inners Catraca.
	 *
	 * @param Inner
	 * @return
	 */
	...
	/**
	 * Libera a catraca no sentido contrário a saída padrão do Inner, para o usuário
	 * poder efetuar o giro na catraca. Em modo On-Line, na função
	 * ReceberDadosOnLine o valor retornado no parâmetro “Complemento” será do tipo
	 * entrada. Essa função deve ser utilizada somente com Inners Catraca.
	 *
	 * @param Inner
	 * @return
	 */
	...
	/**
	 * Prepara o Inner para trabalhar no modo Off-Line, porém essa função ainda não
	 * envia essa informação para o equipamento.
	 *
	 * @return
	 */
	 ...
	 * sentido de entrada do Inner. Caso a mensagem passe de 32 caracteres a DLL irá
	 * utilizar os primeiros 32 caracteres. O Inner não aceita caracteres com
	 * acentuação, padrão UNICODE ou padrão ANSI. O Inner aceita apenas os
	 * caracteres do padrão ASCII. ExibirData: 0 – Não exibe a data/hora na linha
	 ...
	 * Caso seja passado uma string vazia o Inner exibirá a mensagem em branco no
	...
	/**
	 * Configura a mensagem a ser exibida quando o usuário passar o cartão no
	 * sentido de saída do Inner. Caso a mensagem passe de 32 caracteres a DLL irá
	 * utilizar os primeiros 32 caracteres. O Inner não aceita caracteres com
	 * acentuação, padrão UNICODE ou padrão ANSI. O Inner aceita apenas os
	 * caracteres do padrão ASCII. ExibirData: 0 – Não exibe a data/hora na linha
	 * superior do display. 1 – Exibe a data/hora na linha superior do
	 ...
	 * a mensagem passe de 32 caracteres a DLL irá utilizar os primeiros 32
	 * caracteres. O Inner não aceita caracteres com acentuação, padrão UNICODE ou
	 * padrão ANSI. O Inner aceita apenas os caracteres do padrão ASCII. ExibirData:
	 * 0 – Não exibe a data/hora na linha superior do display. 1 – Exibe a data/hora
	 ...
	 * 16 ao invés de 32. Caso seja passado uma string vazia o Inner exibirá a
	 * mensagem em branco no display. Passe o cartão(Default).
	 *
	 * @param ExibirData
	 * @param Mensagem
	 * @return
	 ...
	 * para o Inner. Após executar a função, o buffer com as mensagens é limpo
	 * automaticamente pela dll. Inner: 1 a 32 – Para comunicação serial. 1 a 99 –
	 * Para comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	 * porta fixa.
	 *
	 * @param Inner
	 * @return
	 ...
	 * OnLine e viceversa. Configura o tempo após a comunicação ser interrompida que
	 * está mudança irá ocorrer. Habilita: 0 – Desabilita a mudança(Default). 1 –
	 * Habilita a mudança. 2 – Habilita a mudança automática para o modo OnLine TCP
	 * com Ping, onde o Inner precisa receber o comando PingOnLine para manter-se
	 * OnLine. Tempo - Tempo em segundos para ocorrer a mudança. 1 a 50.
	 *
	 * @param Habilita
	 * @param Tempo
	 * @return
	 ...
	 * “DefinirEntradasMudançaOffLineComBiometria”. Teclado: 0 – Não aceita dados
	 * pelo teclado. 1 – Aceita dados pelo teclado. Leitor1: 0 – desativado 1 –
	 * somente para entrada 2 – somente para saída 3 – entrada e saída 4 – saída e
	 * entrada Leitor2: 0 – desativado 1 – somente para entrada 2 – somente para
	 * saída 3 – entrada e saída 4 – saída e entrada Catraca - 0 – reservado para
	 * uso futuro.
	 *
	 * @param Teclado
	 * @param Leitor1
	 ...
	 * após uma queda para off line. 0 – Não exibe a data/hora na linha superior do
	 ...
	 * mensagem passa a ser 16 ao invés de 32. Caso seja passado uma string vazia o
	...
	/**
	 * Configura a mensagem padrão a ser exibida pelo Inner quando ele mudar para
	 * Off-line. ExibirData: 0 – Não exibe a data/hora na linha superior do display.
	...
	/**
	 * Envia o buffer com as configurações de mudança automática do modo OnLine para
	 * OffLine . Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação
	 * TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 * @return
	 ...
	 * essas funções. Caso a mensagem passe de 32 caracteres a DLL irá utilizar os
	 * primeiros 32 caracteres. O Inner não aceita caracteres com acentuação, padrão
	 * UNICODE ou padrão ANSI. O Inner aceita apenas os caracteres do padrão ASCII.
	 * Mensagem: String com a mensagem a ser exibida. Caso esteja exibindo a
	 * data/hora o tamanho da mensagem passa a ser 16 ao invés de 32. Caso seja
	 * passado uma string vazia o Inner não exibirá a mensagem no display. Funcao -
	 * 0 a 9. Habilitada: 0 – Desabilita a função do Inner(Default). 1 – Habilita a
	 * função do Inner.
	 *
	 * @param Mensagem
	 * @param Funcao
	...
	/**
	 * Coleta um bilhete Off-Line que está armazenado na memória do Inner, os dados
	 * do bilhete são retornados por referência nos parâmetros da função. Ates de
	 * chamar esta função pela primeira vez é preciso chamar obrigatoriamente as
	 * funções DefinirPadraoCartao e DefinirQuantidadeDigitosCartao nessa ordem para
	 * que o número do cartão seja calculado de forma correta. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa. Tipo - Tipo da marcação registrada.
	 * 0 a 9 – Funções registradas pelo cartão. 10 – Entrada pelo cartão. 11 – Saída
	 * pelo cartão. 12 – Tentativa de entrada negada pelo cartão. 13 – Tentativa de
	 * saída negada pelo cartão. 100 a 109 – Funções registradas pelo teclado. 110 –
	 * Entrada pelo teclado. 111 – Saída pelo teclado. 112 – Tentativa de entrada
	 * negada pelo teclado. 113 – Tentativa de saída negada pelo teclado. Dia - 1 a
	 * 31 Mes - 1 a 12 Ano - 0 a 99 Hora - 0 a 23 Minuto - 0 a 59 Cartao - Número do
	 * cartão do usuário.
	 *
	 * @param Inner
	 * @param Dados
	...
	/**
	 * Prepara o Inner para trabalhar no modo On-Line, porém essa função ainda não
	 * envia essa informação para o equipamento.
	 *
	 * @return
	 */
	 ...
	 * ReceberDadosOnLine. Recebe: 0 – Não receber a data/hora do bilhete(Default).
	 ...
	 * caracteres. O Inner não aceita caracteres com acentuação, padrão UNICODE ou
	 * padrão ANSI. O Inner aceita apenas os caracteres do padrão ASCII. Inner: 1 a
	 * 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta
	 * variável. 1 a ... – Para comunicação TCP/IP porta fixa. ExibirData: 0 – Não
	 ...
	 * invés de 32. Caso seja passado uma string vazia o Inner exibirá a mensagem em
	 * branco no display.
	 *
	 * @param Inner
	 * @param ExibirData
	 ...
	 * alguma informação for recebida no modo OnLine através da função
	 * ReceberDadosOnLine, a função EnviarFormasEntradasOnLine deverá ser chamada
	 * novamente para reconfigurar o Inner. Inner: 1 a 32 – Para comunicação serial.
	 * 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a ... – Para
	 * comunicação TCP/IP porta fixa QtdeDigitosTeclado - 0 a 20 dígitos.
	 * EcoTeclado: 0 – para não 1 – para sim 2 – ecoar FormaEntrada: 0 – não aceita
	 ...
	 * 1 e leitor 2 7 – teclado, leitor 1 e leitor 2 10 – teclado + verificação
	 * biométrica 11 – leitor1 + verificação biométrica 12 – teclado + leitor1 +
	 * verificação biométrica 13 – leitor1 com verificação biométrica + leitor2 sem
	 * verificação biométrica 14 – leitor1 com verificação biométrica + leitor2 sem
	 * verificação biométrica + teclado sem verificação biométrica 100 – Leitor 1 +
	 * Identificação Biométrica (sem Verificação) 101 – Leitor 1 + Teclado +
	 * Identificação Biométrica (sem Verificação) 102 – Leitor 1 + Leitor 2 +
	 * Identificação Biométrica (sem Verificação) 103 – Leitor 1 + Leitor 2 +
	 * Teclado + Identificação Biométrica (sem Verificação) 104 – Leitor 1 invertido
	 * + Identificação Biométrica (sem Verificação) 105 – Leitor 1 invertido +
	 * Teclado + Identificação Biométrica (sem Verificação) TempoTeclado - 1 a 50
	 ...
	 * cartão no Inner retorna as informações do cartão nos parâmetros da função.
	 * Para que a data/hora do bilhete OnLine seja retornada, o Inner deverá ter
	 * sido previamente configurado através da função ReceberDataHoraDadosOnLine.
	 * Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com
	 * porta variável. 1 a ... – Para comunicação TCP/IP porta fixa. Origem - Origem
	 ...
	 * catraca Topdata (sensor ótico) 7 – Urna (catraca Millenium) 8 – Evento no
	 ...
	 * entrada (com cartão) 35 – # via teclado (1° tecla) 42 – * via teclado (1°
	 * tecla) 65 – “Função” via teclado 66 – “Entrada” via teclado 67 – “Saída” via
	 * teclado 255 – Inseriu todos os dígitos permitidos pelo teclado. Evento do
	 * Sensor 0/1 – Nível atual do sensor Cartao - Número do cartão recebido. Dia -
	 ...
	 * Line após uma queda para OffLine. Entrada: 0 – Não aceita entrada de dados. 1
	...
	/**
	 * Habilita/Desabilita a identificação biométrica e/ou a verificação biométrica
	 * do Inner bio. O resultado da configuração deve ser obtivo através do comando
	 * ResultadoConfiguracaoBio. Inner: 1 a 32 – Para comunicação serial. 1 a 99 –
	 * Para comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	...
	/**
	 * Retorna o resultado da configuração do Inner Bio, função ConfigurarBio. Se o
	 * retorno for igual a 0 é poque o Inner bio foi configurado com sucesso. Inner:
	 * 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta
	 * variável. 1 a ... – Para comunicação TCP/IP porta fixa. OnLine: 0 – O comando
	...
	/**
	 * Insere o número do cartão na lista de usuários sem digital do Inner bio. Este
	 * número ficara armazenado em um buffer interno dentro da dll e somente será
	 * enviado para o Inner após a chamada a função
	 * EnviarListaUsuariosSemDigitalBio. O número máximo de dígitos para o cartão é
	 * 10, caso os cartões tenham mais de 10 dígitos, utilizar os 10 dígitos menos
	 * significativos do cartão. Cartao - 1 a 9999999999 – Número do cartão do
	 * usuário.
	 *
	 * @param Cartao
	 * @return
	...
	/**
	 * Solicita a versão do firmware da placa do Inner Bio, a placa que armazena as
	 * digitais. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação
	 * TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 * @return
	...
	/**
	 * Solicita o modelo do Inner bio. Para receber o resultado dessa operação você
	 * deverá chamar a função ReceberModeloBio enquanto o retorno for processando a
	 * operação. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação
	 * TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 * @return
	...
	/**
	 * Retorna o resultado do comando SolicitarModeloBio, o modelo do Inner Bio é
	 * retornado por referência no parâmetro da função. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 ...
	 * Modelo: 2 – Bio Light 100 usuários. 4 – Bio 1000/4000 usuários. 255 – Versão
	 * desconhecida.
	 *
	 * @param Inner
	 * @param OnLine
	...
	/**
	 * Retorna o resultado do comando SolicitarVersaoBio, a versão do Inner Bio é
	 * retornado por referência nos parâmetros da função. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa. OnLine: 0 – O comando foi utilizado
	 * com o Inner em OffLine. 1 – O comando foi utilizado com o Inner em OnLine
	 * VersaoAlta - Parte alta da versão. VersaoBaixa - Parte baixa da versão.
	 *
	 * @param Inner
	 * @param OnLine
	 * @param Versao
	...
	/**
	 * Define que o Inner utilizado no momento é um Inner bio light ao invés de um
	 * Inner bio 1000/4000. Essa função deverá ser chamada sempre que necessário
	 * antes das funções SolicitarUsuarioCadastradoBio, SolicitarExclusaoUsuario,
	 * InserirUsuarioLeitorBio e FazerVerificacaoBiometricaBio. Light: 1 – É um
	 * Inner bio light 0 – É um Inner bio 1000/4000(valor default)
	 *
	 * @param Light
	 */
	 ...
	 * biométrico. O leitor irá acender a luz vermelho e após o usuário inserir a
	 * digital, automaticamente o usuário será cadastrado no nner bio com o número
	 * do cartão passado no parâmetro Usuário. Inner: 1 a 32 – Para comunicação
	 * serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a ... – Para
	 * comunicação TCP/IP porta fixa. Tipo: 0 – para solicitar o primeiro template 1
	 * – para solicitar o segundo template (mesmo dedo) e salvar. 2 – para solicitar
	 * o segundo template (outro dedo) e salvar. Usuario - Número do cartão que o
	 * usuário terá.
	 *
	 * @param Inner
	 * @param Tipo
	 ...
	 * igual a 0 é porque o usuário foi cadastrado com sucesso. Inner: 1 a 32 – Para
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa. OnLine: 0 – O comando foi utilizado
	 * com o Inner em OffLine. 1 – O comando foi utilizado com o Inner em OnLine.
	 *
	...
	/**
	 * Solicita para o Inner bio excluir o cadastro do usuário desejado. O Retorno
	 * da exclusão é verificado através da função UsuarioFoiExcluido Inner: 1 a 32 –
	 * Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável.
	 * 1 a ... – Para comunicação TCP/IP porta fixa. Usuario - Número do cartão do
	 * usuário cadastrado.
	 *
	 * @param Inner
	 * @param Usuario
	 ...
	 * função for igual a 0 é porque o usuário foi excluído com sucesso. Inner: 1 a
	 * 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta
	 * variável. 1 a ... – Para comunicação TCP/IP porta fixa. OnLine: 0 – O comando
	 * foi utilizado com o Inner em OffLine. 1 – O comando foi utilizado com o Inner
	 * em OnLine.
	 ...
	 * Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com
	 * porta variável. 1 a ... – Para comunicação TCP/IP porta fixa. Usuario -
	 ...
	 * as duas digitais do usuário cadastrado no Inner Bio. O template é retornado
	 * por referência nos parâmetros da função. Inner: 1 a 32 – Para comunicação
	 * serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a ... – Para
	 ...
	 * Cadastro do usuário com as duas digitais, o dado está em binário e não deve
	 * ser alterado nunca. O tamanho do template é de 844 bytes.
	 *
	 * @param Inner
	 * @param OnLine
	 * @param Template
	 ...
	 * de usuários cadastrados no Inner Bio é retornado por referência nos
	 * parâmetros da função. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para
	 * comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	 * porta fixa. OnLine: 0 – O comando foi utilizado com o Inner em OffLine. 1 – O
	 * comando foi utilizado com o Inner em OnLine Quantidade - Total de usuários
	...
	/**
	 * Prepara a dll para iniciar a coleta dos usuários do Inner bio, essa função
	 * deve ser chamada obrigatoriamente no início do processo.
	 */
	public static native void InicializarColetaListaUsuariosBio();

	/**
	 * Retorna 1 se ainda existe mais pacotes da lista de usuários, a ser recebido
	 * do Inner Bio.
	 *
	 * @return
	 */
	 ...
	 * comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta variável. 1 a
	 * ... – Para comunicação TCP/IP porta fixa. Usuario - Número do cartão do
	 * usuário cadastrado.
	 *
	 * @param Inner
	 * @param Usuario
	...
	/**
	 * Solicita o pacote(a parte) atual da lista de usuários do Inner bio. Inner: 1
	 * a 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta
	...
	/**
	 * Receber o pacote solicitado pela função SolicitarListaUsuariosBio. Inner: 1 a
	 * 32 – Para comunicação serial. 1 a 99 – Para comunicação TCP/IP com porta
	 * variável. 1 a ... – Para comunicação TCP/IP porta fixa.
	 *
	 * @param Inner
	 ...
	 * de dados. O resultado do cadastro deve ser verificado no retorno da função
	 * UsuarioFoiEnviado. Inner: 1 a 32 – Para comunicação serial. 1 a 99 – Para
	 * comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	 * porta fixa. Template: O cadastro do usuário já contendo as duas digitais e o
	 * número do usuário. É um array de bytes com o tamanho de 844 bytes.
	 ...
	 * EnviarUsuarioBio. Se o retorno for igual a 0 é porque o template foi
	 * cadastrado com sucesso. Inner: 1 a 32 – Para comunicação serial. 1 a 99 –
	 * Para comunicação TCP/IP com porta variável. 1 a ... – Para comunicação TCP/IP
	 * porta fixa. OnLine: 0 – O comando foi utilizado com o Inner em OffLine. 1 – O
	...
	/**
	 * Configura as formas de entradas de dados para quando o Inner mudar para o
	 * modo Off-Line. Esse comando difere do anterior por permitir a configuração de
	 * biometria. Através dessa função o Inner pode ser configurado para trabalhar
	 * com verificação ou identificação biométrica, quando ocorrer uma mudança
	 * automática de On-Line para Off-Line. Teclado: 0 – Não aceita dados pelo
	 * teclado. 1 – Aceita dados pelo teclado. Leitor1: 0 – desativado 3 – entrada e
	 * saída 4 – saída e entrada (nesse caso força Leitor2 igual a zero)</param>
